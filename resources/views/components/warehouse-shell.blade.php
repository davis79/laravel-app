@props(['title', 'description' => null, 'rootLabel' => 'Magazyn owocowy', 'rootRoute' => 'warehouse.products'])
...
<div class="min-h-screen bg-[#060818] text-slate-300">
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 -translate-x-full border-r border-white/[0.06] bg-[#0d1024] transition-transform duration-300 lg:translate-x-0">
        ...
        <nav class="h-[calc(100vh-5rem)] overflow-y-auto px-4 py-6">
            ...
            <a href="{{ route('warehouse.products') }}" class="mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm {{ request()->routeIs('warehouse.*') ? 'bg-indigo-500/15 font-semibold text-indigo-300 ring-1 ring-inset ring-indigo-500/20' : 'text-slate-400 hover:bg-white/[0.04] hover:text-white' }}"><svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 7h16v13H4zM7 7V4h10v3M8 11h8M8 15h5"/></svg>Magazyn owocowy</a>
            <a href="{{ route('vaccines.index') }}" class="mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm {{ request()->routeIs('vaccines.*') ? 'bg-indigo-500/15 font-semibold text-indigo-300 ring-1 ring-inset ring-indigo-500/20' : 'text-slate-400 hover:bg-white/[0.04] hover:text-white' }}"><svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M9 3h6v3l2 2v12H7V8l2-2V3ZM9 12h6M12 9v6"/></svg>Szczepionki</a>
            <a href="{{ route('freezers.index') }}" class="mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm {{ request()->routeIs('freezers.*') ? 'bg-indigo-500/15 font-semibold text-indigo-300 ring-1 ring-inset ring-indigo-500/20' : 'text-slate-400 hover:bg-white/[0.04] hover:text-white' }}"><svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 2v20M4.5 6.5l15 11M19.5 6.5l-15 11M4 12h16"/></svg>Zamrażarki</a>
            ...
            <a href="#" class="mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-slate-400 hover:bg-white/[0.04] hover:text-white"><span class="size-2 rounded-full border border-slate-600"></span>Karty produkcyjne</a>
            <a href="#" class="mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-slate-400 hover:bg-white/[0.04] hover:text-white"><span class="size-2 rounded-full border border-slate-600"></span>Planowanie</a>
            ...
            <a href="{{ route('settings.index') }}" class="mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm {{ request()->routeIs('settings.*') ? 'bg-indigo-500/15 font-semibold text-indigo-300 ring-1 ring-inset ring-indigo-500/20' : 'text-slate-400 hover:bg-white/[0.04] hover:text-white' }}"><span class="size-2 rounded-full border border-current"></span>Ustawienia</a>
        </nav>
    </aside>
    ...
    <div class="lg:pl-64">
        ...
        <main class="p-4 sm:p-8"><div class="mx-auto max-w-[1500px]">
            <div class="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div class="mb-3 flex gap-2 text-xs text-slate-500"><a href="{{ route($rootRoute) }}">{{ $rootLabel }}</a>{{ $breadcrumbs ?? '' }}</div><h1 class="text-2xl font-bold text-white sm:text-3xl">{{ $title }}</h1>@if($description)<p class="mt-2 text-sm text-slate-500">{{ $description }}</p>@endif</div>{{ $actions ?? '' }}</div>
